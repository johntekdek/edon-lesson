...
2. **Player** — a self-contained embeddable JavaScript bundle (script + mount API, no host framework) rendering Published Versions inside Moodle activity pages and Drafts inside the Authoring UI's Preview overlay (FR-9: same Player, same script). Governed by a strict bundle budget and the Android 8-era device floor (FR-18, OQ-16). No component library, no shadows; the *core* bundle ships no webfonts (`{typography.player-ui}`) — Showcase-tier polish (brand fonts, motion) arrives only as lazy enhancement chunks that never count against the core budget (stakeholder amendment, 2026-07-17).
...
The device floor (OQ-16: ~1.5–2 GB RAM, Android 8-era, outdated WebViews, metered data) is a first-class design target. Degraded states are **designed states, not error states** — the poster path gets the same design attention as the full path, and nothing on it may look broken.

| Tier | Detection (feature-based, never user-agent) | Model3D | Simulation | Narration |
|---|---|---|---|---|
| Showcase | Capable device + fast connection, auto-detected; any data-saver signal, metered hint, or detection uncertainty demotes — when unsure, demote (stakeholder amendment, 2026-07-17) | Auto-loads with the hero entrance (slow ~2s auto-orbit, settles; inside the reserved media box; skipped under `prefers-reduced-motion`) | Auto-loads to its ready state — sound and narration still start only on explicit action | Same as Full; Player text (chrome and content) upgrades to brand webfonts via the lazy enhancement chunk (`{typography.player-ui}`) |
| Full | WebGL available + asset within its per-Block budget | Poster first; "Load 3D model (size)" → interactive viewer | Poster first; "Run simulation" → sandboxed run | SpeechSynthesis play control + "Show text" |
...
- **Tap-to-load below Showcase** (amended by stakeholder, 2026-07-17). On Full, Constrained, and Floor tiers, heavy assets (Model3D, Simulation) never auto-load: the poster renders first with an explicit load control showing the download size — respecting metered data and keeping the poster path a routinely exercised rendering path, not a rarely-tested branch. On Showcase, heavy Blocks auto-load; the poster remains the instant first paint while the asset arrives, and any load failure lands on the normal Constrained behavior.
- **Showcase is an enhancement, never a dependency.** Its webfonts and motion ship as lazy chunks after first content paint, never counted against the core bundle budget and never render-blocking. Demotion is graceful mid-session: if the connection degrades, the next heavy Block simply behaves like Full. Motion never touches gold governance signals, and every animation respects `prefers-reduced-motion`. The Floor tier, poster paths, and durability rules are unchanged by this tier and remain first-class.
...
- All Player chrome uses `DESIGN.md` colors and, in the core bundle, the system font stack; on Showcase the lazy enhancement chunk upgrades Player text to the brand fonts per `{typography.player-ui}`. The Player root carries its own opaque background and hairline border, so it reads as a deliberate card on light institutional themes (light-only confirmed — item 4 in § Rulings & Open Items; a dark-theme audit is post-MVP).
...
| Notice banner | Both surfaces | Info: confirmations and idempotency notices. Warning (gold): Budget Ceiling pause — generation disabled with honest copy, replay unaffected (OQ-9/FR-26); Tenant Admin editing another Teacher's lesson. Error: failed jobs, writeback problems. Banners are dismissible except the budget pause, which persists while true; the disabled Generate form is programmatically associated with the budget banner so the *reason* is discoverable by assistive tech. |
...
| Narration control | Player | Explicit play/pause; never autoplays. "Show text" toggles the transcript and exposes its pressed/expanded state to AT; when SpeechSynthesis is unusable, transcript shows by default and the play control is hidden (A-9). If speech does not audibly start within a bounded time of tap, the control converts to that same Floor state — quiet, never an error (Android WebViews routinely report voices that then fail silently). Narration state resets per Block. |
...
| Session ending (Launch Token near expiry) | Review Workspace | If the token lifetime is known, a warning banner precedes expiry ("Your session ends soon — recent changes are saved"); expiry itself lands on the Relaunch notice, and relaunch returns the Teacher into the same Draft, not Course home. Token lifetime and warning lead time: Open Item 16. |
...
- 3D orbit/zoom gestures activate only once the asset is interactive (after tap-to-load below Showcase, after auto-load on Showcase), inside the canvas; everything the canvas does is also available as buttons (zoom ±, reset view).
...
| Performance as UX | Text renders before assets; every >2s wait has a labeled state; heavy assets are tap-to-load with transfer size shown below Showcase (Showcase auto-loads — the poster is still the instant first paint). Measurable, CI-bound floors: lesson title + first Slide text visible within an architecture-set budget *under the throttled CI low-spec profile* (NFR-2's existing rig — the measurement instrument is already mandated); skeletons appear only for waits >300ms, are static below Showcase (tiered motion budget in `DESIGN.md`), and always resolve text-first. The *core* Player bundle budget is unchanged by the Showcase tier and these timing floors apply to every tier including Showcase; Showcase enhancement chunks (webfonts, motion) load lazily after first content paint and never count against the core budget. All of it is a UX guarantee enforced in CI, not an engineering nicety. |
...
3. The Simulation Block shows its poster and **Run simulation (1.1 MB)**. He taps; sliders appear; dragging resistance changes the current live (FR-17).
...
5. **Tap-to-load — RESOLVED as amended:** applies below the Showcase tier only; Showcase auto-loads heavy Blocks (amendment E).
