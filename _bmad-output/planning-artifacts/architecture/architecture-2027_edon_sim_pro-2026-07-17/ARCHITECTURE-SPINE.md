...
- **Rule:** consumers call one async provider-agnostic adapter keyed by workload (`lesson_generation`, `simulation_generation`, `diagram_generation`, **`tts`** — added by stakeholder amendment 2026-07-18 for publish-time narration audio; the `embeddings` key exists in config for completeness but the platform never calls it — embeddings stay inside edon-rag); provider/model/params come from per-workload config. The adapter exposes two surfaces under the same config/telemetry/governance discipline: OpenAI-compatible chat-completions (text workloads) and an OpenAI-compatible speech surface (`tts`) — an additive extension of §3, not a second adapter. Internally exactly two drivers: `openai-compatible` (base_url per provider: OpenAI, Gemini compat, vLLM, OpenRouter) and `anthropic-native` (translating — Anthropic's compat layer is test-grade and drops structured outputs/caching; verified 2026-07-17). The adapter request type has **no identity fields**; it carries a mandatory `governance_ref = (action_type, accounting_ref)` — still identity-free — so reservation keys on `action_type` (e.g. `lesson_generation` vs `block_regeneration` sharing one workload) and only the telemetry writer resolves `accounting_ref` to tenant/pseudonym. Cost Telemetry and governance (AD-8) execute in this call path — there is no ungoverned way to spend.
...
- **Rule:** PostgreSQL is the system of record; Lesson Scripts are JSONB validated before every write. `published_versions` is immutable — the app role has no UPDATE/DELETE grant. One mutable Draft per Lesson with a monotonic `revision` for optimistic concurrency (`If-Match`; mismatch = 409). Citations live embedded in the script (self-containment) *and* projected to a query table; the embedded copy is authoritative. Publishing is an explicit action running as a short **publish job** (progress-visible): revalidate → pre-publish checks → **TTS generation for changed Narration Blocks only** (`tts` workload; per-Block text hash decides "changed", so republish regenerates only changed audio) → asset freeze (audio included in the manifest) → one final transaction asserting the checked `revision` is unchanged (AD-23) → version insert → event. Audio is a static per-lesson asset: per-lesson cost at publish, zero per-student cost (I-1 holds).
...
- **Rule:** every mutation goes through a core use-case service; FR-27 Structured Events are inserted in the same transaction as the state they record. The event taxonomy is closed and canonical: the FR-27 list + the UX-approved extensions (`diagram_review_completed`, `diagram_invalidated`) + three ratified architecture additions (`operator_action`, `cost_alert`, `writeback_overdue` — stakeholder sign-off 2026-07-18, synced into PRD FR-27); Regeneration emits the `lesson_generated`/`generation_failed` family with `scope: lesson|block` detail. New event types require a PRD/architecture note, not ad-hoc strings.
...
- **Rule:** (a) `BlockRegistry` maps `block.type` → renderer; unknown types are omitted from sequence and counts — new types (incl. dialogue turns) are registry entries + a schema minor bump. (b) The Player consumes scripts only via `LessonDeliverySource` — MVP `CompleteScriptSource` resolves the full Published Version, and the interface exposes an async block-iterator signature so incremental delivery is a drop-in source, not a Player change (backend-side, streaming is an additive new endpoint). (c) Narration only via `NarrationProvider` (`available/speak/pause/stop/events`); MVP registers **two providers**: `PregeneratedAudioProvider` (plays the Block's `audioRef` asset — primary when present) and `SpeechSynthesisProvider` as fallback (bounded-start watchdog: no audible start ≤ 3 s → transcript rung; WebView-shell browsers lack Web Speech entirely — verified — and land there by detection). **Text is always the primary modality** — audio is an enhancement, never promised universal (stakeholder positioning, 2026-07-18). (d) Results (view marks, quiz submissions, completion) leave the Player only via `ResultsSink` — MVP `PlatformResultsSink` (playback API + AD-15 outbox semantics); preview mounts a no-op sink, so preview can never write results. No runtime schema validation in the Player (scripts arrive server-validated). Rationale record: ADR-014.

### AD-11 — Embedding, device posture, and budget contract
- **Binds:** /player build + mod_edonlesson embed (FR-18, FR-22, NFR-2, UX Embedding Contract; device posture per stakeholder realignment 2026-07-18, superseding the OQ-16 ruling)
- **Prevents:** the embed fighting Moodle pages; budgets and tier behavior forking between pipeline, player, and CI
- **Rule:** the Player ships as one self-contained IIFE exposing `EdonPlayer.mount(el, opts)`/`unmount`; syntax target ES2017 (`build.target`/`cssTarget: 'chrome61'`) with targeted built-in polyfills (`globalThis`, `queueMicrotask`, `Promise.finally`, + compat-lint findings) and no `import.meta.url` in IIFE output (base URL via `document.currentScript`). Styles namespaced `edon-p-*`, container-scoped, no global resets; in-page mount (no iframe at Player level — the Embedding Contract's scroll/height rules). **Device posture: Showcase/Full is the canonical, default experience** — modern smartphones with reliable connectivity are assumed; heavy assets **auto-load by default on every tier**, and tap-to-load (with transfer size label) applies **only under explicit data-saver signals** (`saveData`, `prefers-reduced-data`, metered hints). **Brand webfonts are standard** — loaded post-first-paint, never render-blocking, on all capable devices. The Floor/Constrained rungs are retained as **thin best-effort fallback** (poster paths and durability rules stay — they also serve flag-off states and future offline export) but do not gate launch: the low-spec CI profile (CPU 6× + 400 kbps/400 ms RTT + ~1.5 GB memory cap) is **advisory**; the **blocking** perceived-performance check runs on a standard modern profile (unthrottled Chromium, 4G-shaped network): title + first Slide text ≤ 2.5 s. The old-engine smoke and Constrained predicate (low class = `deviceMemory ≤ 2` or undefined → halved asset ceiling; any load failure/timeout → Constrained behavior) remain as fallback mechanics. All budgets live in one repo-root `budgets.json` — own JSON Schema in `/schema`, values in **bytes** (ms where temporal), consumed by exactly four readers: player CI, pipeline validators, the ingest CLI, and the simulation check harness. Launch values (raised per the realignment, lazy-chunk discipline retained): core ≤ 180 kB gzip (hard fail); lazy chunks — `model3d` ≤ 250 kB gzip, `simulation` ≤ 60 kB, `fonts+motion` ≤ 300 kB (standard, post-first-paint); assets — glTF ≤ 10 MB transfer hard cap (≤ 6 MB preferred at selection), posters ≤ 100 kB, diagram SVG ≤ 200 kB, **narration audio ≤ 800 kB per Block / ≤ 10 MB per lesson**. At most one heavy Block live at a time (still cheap insurance on any hardware).
...
| React / react-dom | 19.2.x (Authoring + Player; Preact rejected for MVP 2026-07-18 — documented future optimisation, measured saving ~50 kB gzip core) |
...
edon-lesson/
  ...
  docs/
    ...
    runbooks/              # deploy, backup/restore, key rotation, tenant onboarding, weekly ops checklist
...
| Voiced lessons — publish-time TTS (stakeholder amendment 2026-07-18) | publish job `tts` stage + `PregeneratedAudioProvider` | AD-2 (`audioRef`), AD-3 (`tts` workload), AD-5 (publish job), AD-10, AD-11 (audio budgets) |
| Live Q&A — course-scoped chat embed (stakeholder amendment 2026-07-18) | block_edon_ai on the activity page, beside the Player | integrations contract WI-CHAT-4; NFR-9 unchanged; zero new inference economics |

### Release & milestones (stakeholder ruling, 2026-07-18)

Single full-MVP release. The walking-skeleton sequence becomes named internal milestone gates, each ending with a **private pilot-college preview**: **M1** schema + generation skeleton (WI-RAG-0 delivered 2026-07-18 — edon-rag v2.1.0, contract v1.2; the live-retrieval slice is unblocked and recorded fixtures remain the CI golden path — status delta authorized at readiness sign-off) · **M2** review/publish + Player core **+ publish-time TTS** · **M3** Moodle integration + grades, **incl. verifying Player style isolation against the production theme** (F4) · **M4** diagrams **+ the course-scoped chat embed** · **M5** Model3D + template Simulations (**blocked until the curated seed library exists: ≥ 20 models across 2–3 NCE science subjects, stakeholder-curated, licence metadata mandatory — OQ-6 resolution**) · **M6** Showcase-canonical polish. Launch = sponsored/paid pilot of ≤ 5 colleges; 60-college scale is post-pilot. In-scope deliverable: a one-page **weekly ops checklist** runbook (spend alerts, `writeback_overdue` review, diagram-report queue, key rotations).
...
| (c) Narration providers | `NarrationProvider` registry (AD-10) — **now exercised by two MVP providers** (pre-generated TTS audio via `audioRef` + SpeechSynthesis fallback), proving the seam | streaming TTS = a third provider registration; no Player change |
...
| Curated Model Library seeding — **OQ-6 RESOLVED 2026-07-18**: ≥ 20 models, 2–3 NCE science subjects, stakeholder-curated, licence metadata mandatory | content work with a fixed bar; ingest pipeline (ADR-012) ready | blocks M5 Model3D stories until the seed bar is met |
