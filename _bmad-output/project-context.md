...
- **edon-rag contract v1.2** (v1.1 at sign-off): retrieval is course-scoped (`POST /api/courses/{moodle_course_id}/retrieve`, `x-api-key` auth). **Status delta (2026-07-18, authorized at readiness sign-off): WI-RAG-0 is delivered** — the retrieval endpoint is live in production (edon-rag v2.1.0); the live-retrieval slice is unblocked; recorded fixtures remain the CI golden path.
