...
3. **`NarrationProvider`** (seam c): `available() / speak(text, lang) / stop() / events`; MVP registers **two providers** (stakeholder amendment 2026-07-18): `PregeneratedAudioProvider` — primary when the Block carries `audioRef` (publish-time TTS audio) — and `SpeechSynthesisProvider` as fallback, including the bounded-start watchdog (3 s) and voice-selection chain (en-NG → en-GB → any en); the transcript rung is a provider reporting unavailability, not an error path. Text is always the primary modality. Streaming TTS = a third provider registration.
