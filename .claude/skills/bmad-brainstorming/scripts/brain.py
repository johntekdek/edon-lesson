...
def category_style(cat: str) -> tuple[str, str]:
    """(hue, glyph markup) for a category — from the sidecar for the shipped set,
    derived for extras."""
...
<style>
  ...
  :root[data-theme="dark"] label.tech:has(input:checked) {
  ...
  .themebtn {
    ...
    background:var(--control); color:var(--ink); font-size:17px; line-height:1;
  ...
  * { box-sizing:border-box; }
  ...
  header {
    position:sticky; top:0; z-index:5; background:var(--surface); padding:20px 0 12px;
    border-bottom:1px solid var(--line); box-shadow:0 2px 12px var(--shadow);
  }
  /* align header content with the card column on wide screens */
  ...
  .modes {
    display:inline-flex; background:var(--control); border-radius:9px; padding:3px; gap:2px;
  }
  .mode {
  ...
  #copy {
    margin-left:auto; padding:9px 22px; background:var(--accent);
  ...
  .chip {
    font-size:12px; padding:4px 11px; border-radius:16px; border:0;
    color:#fff; background:var(--cc); font-weight:600; cursor:pointer;
  }
  .chip:hover { filter:brightness(1.08); }
  .banner {
    max-height:0; overflow:hidden;
    transition:max-height .25s ease, padding .22s ease, margin .22s ease;
    background:linear-gradient(90deg,var(--accent),#8275f2); color:#fff;
    border-radius:10px; font-weight:700; text-align:center; padding:0 14px;
  }
  ...
  section > h2 {
    font-size:13px; text-transform:uppercase; letter-spacing:.08em; color:var(--c);
    margin:0 0 10px; border-bottom:1px solid color-mix(in srgb, var(--c) 24%, var(--line));
    padding-bottom:6px;
  }
  ...
  .grouphdr {
    margin:30px 0 12px; font-size:12px; text-transform:uppercase; letter-spacing:.14em;
    font-weight:700; color:var(--c); opacity:.92;
    border-bottom:1px solid color-mix(in srgb, var(--c) 22%, var(--line));
    padding-bottom:7px;
  }
  ...
  .goal {
    ...
    background:var(--control); color:var(--muted); font-weight:600;
  }
...
<header>
  ...
  <p class="sub">Compose your session, hit <strong>Copy prompt</strong>,
    and paste it back into the chat to begin. {{TOTAL}}</p>
  ...
  <div class="composer">
    ...
    <div class="grp">
      ...
      <span class="step">Random <button type="button" data-step="rand"
        data-d="-1">&minus;</button><b id="randN">0</b><button type="button"
        data-step="rand" data-d="1">+</button></span>
      <span class="step">Invent <button type="button" data-step="inv"
        data-d="-1">&minus;</button><b id="invN">0</b><button type="button"
        data-step="inv" data-d="1">+</button></span>
      <span class="step">AI picks <button type="button" data-step="ai"
        data-d="-1">&minus;</button><b id="aiN">0</b><button type="button"
        data-step="ai" data-d="1">+</button></span>
  ...
  <div class="banner" id="banner">&#10003; Copied!
    Now paste it into the chat to start your session.</div>
  </div>
</header>
<main>
{{BODY}}
...
(function(){
  ...
  // per-category "invent in the spirit of" cards
  var inventBoxes = boxes.filter(function(b){ return b.dataset.invent; });
  ...
  var MODE_HINTS = {
    ...
    'Creative Partner': 'We riff together — I facilitate and add ideas too, ' +
      'each logged as yours or mine.',
    'Ideate for me': 'I run the whole session myself, then show you the result ' +
  ...
  all.call(document.querySelectorAll('.mode')).forEach(function(b){
    b.addEventListener('click', function(){
      all.call(document.querySelectorAll('.mode')).forEach(function(m){
  ...
  all.call(document.querySelectorAll('.chip')).forEach(function(chip){
    chip.addEventListener('click', function(){
      ...
      for (var i = 0; i < sections.length; i++){
        if (sections[i].dataset.cat === chip.dataset.cat){ sec = sections[i]; break; }
      }
      if (!sec){ return; }
  ...
  function activeGoals(){
    return goalBtns.filter(function(b){ return b.classList.contains('on'); })
      .map(function(b){ return b.dataset.goal; });
  }
  ...
  goalBtns.forEach(function(b){
    b.addEventListener('click', function(){ b.classList.toggle('on'); applyFilter(); });
  });
  ...
  function randomPool(){
    ...
    return techBoxes.filter(function(b){
      return !b.checked && !b.dataset.lead && !picked[b.dataset.name];
    });
  }
  ...
  function compose(){
    var picks = checkedTech().map(function(b){
      return { n: b.dataset.name, c: b.dataset.cat, d: b.dataset.desc, r: false };
    });
    var rnd = sample(randomPool(), state.rand).map(function(b){
      return { n: b.dataset.name, c: b.dataset.cat, d: b.dataset.desc, r: true };
    });
    ...
    var extra = [];
    if (state.inv > 0){
      extra.push('invent ' + state.inv + ' brand-new technique' +
        (state.inv > 1 ? 's' : '') + ' on the fly');
    }
    checkedInvent().forEach(function(b){
      extra.push('invent 1 new technique in the spirit of ' + b.dataset.invent);
    });
    if (state.ai > 0){
      extra.push('you choose ' + state.ai + ' more technique' +
        (state.ai > 1 ? 's' : '') + ' that fit my goal');
    }
    if (extra.length){ L.push("", 'Then: ' + extra.join('; and ') + '.'); }
    if (!techs.length && !extra.length){
      L.push("", state.mode === 'Ideate for me'
        ? 'Run the whole session yourself — pick the techniques, ' +
          'generate the ideas, then show me the result.'
...
def _svg(inner: str) -> str:
    return (
        f'<svg class="ico" viewBox="0 0 44 44" '
        f'xmlns="http://www.w3.org/2000/svg">{CHIP}{inner}</svg>'
    )
...
def _card(r: dict, lead: bool = False) -> str:
    ...
    return (
        ...
        f'data-name="{name}" data-cat="{disp_cat}" data-desc="{desc}" '
        f'data-good="{good}" data-prov="{prov}"{lead_attr}>'
...
def _invent_card(disp_cat: str, glyph: str) -> str:
    ...
    return (
        ...
        f'<span class="d">Make up a brand-new technique on the fly, '
        f"in the spirit of {disp_cat}</span></span></label>"
    )
...
def html_doc(rows: list[dict]) -> str:
    ...
    def add_section(cat: str) -> None:
        ...
        chips.append(
            ...
            f'data-cat="{disp}" style="--cc:{hue}">{disp}</button>'
        )
        body.append(
            f'<section data-cat="{disp}" style="--c:{hue}">'
            f'<h2>{disp}<span class="cnt">{len(groups[cat])}</span></h2>'
    ...
    if classics:
        ...
        chips.append(
            f'<button type="button" class="chip" '
            f'data-cat="{disp}" style="--cc:{LEAD_HUE}">{disp}</button>'
        )
        body.append(
            f'<section data-cat="{disp}" style="--c:{LEAD_HUE}">'
            f'<h2>{disp}<span class="cnt">{len(classics)}</span></h2>'
    ...
    if present_goals:
        ...
        gchips = "".join(
            f'<button type="button" class="goal" data-goal="{html.escape(g)}">'
            f"{html.escape(GOAL_LABELS.get(g, g))}</button>"
            for g in ordered
        )
        goalbar = (
            f'<div class="bar"><span class="glabel">Great for</span>'
            f'<div class="goals" id="goals">{gchips}</div></div>'
        )
...
def main(argv: list[str] | None = None) -> int:
    ...
    p.add_argument(
        ...
        type=Path,
        help=(
            "JSON overlay of additional techniques (customize.toml "
            "additional_techniques), merged into every command"
        ),
    )
    ...
    if args.cmd == "categories":
        ...
        if not args.category and not args.all:
            print(
                ...
                "catalog on purpose. Use `categories` for the cheap map, or `random` to "
                "draw blind.",
                file=sys.stderr,
            )
            return 2
