...
- `published_versions` — **immutable**: `(lesson_id, version_no)` unique, full `script` JSONB (citations embedded), `schema_version`, `asset_manifest` JSONB, `published_at`, `published_by`. No UPDATE/DELETE grants on this table for the app role — immutability is enforced by the database, not convention. Publishing = transaction: revalidate Draft → run pre-publish checks (FR-17/A-35) → freeze assets (ADR-012) → insert version row → emit `lesson_published` event.
