...
| Narration control | Compact bar under the slide heading: play/pause (44px target), elapsed dots, "Show text" toggle. Uses `{colors.primary}` glyphs on `{colors.surface}`; active state `{colors.primary-wash}`. Transcript renders as normal body text below. |
...
| Diagram chat message | Chat bubble containing: AI content label, sanitised SVG (white card, `{rounded.md}`), caption row with "Report this diagram" quiet-destructive text button — `{typography.body-sm}` label on a ≥44px hit area (it is the FR-28 governance control, not metadata). |
