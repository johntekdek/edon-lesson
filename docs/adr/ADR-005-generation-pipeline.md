...
- **Retrieval is course-scoped** (edon-rag contract v1.2): `POST /api/courses/{moodle_course_id}/retrieve`, `x-api-key`-authed; the course id comes from the Launch Token's `course_ref`. **Status delta (2026-07-18, authorized at readiness sign-off): WI-RAG-0 is delivered** — the retrieval endpoint is live in production (edon-rag v2.1.0); the live-retrieval slice is unblocked, and recorded fixtures remain the CI golden path.
